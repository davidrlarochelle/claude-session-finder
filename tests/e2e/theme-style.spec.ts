...
const htmlAttr = (page: Parameters<typeof waitForList>[0], attr: string): Promise<string | null> =>
  page.evaluate((a) => document.documentElement.getAttribute(a), attr);
const hasDark = (page: Parameters<typeof waitForList>[0]): Promise<boolean> =>
  page.evaluate(() => document.documentElement.classList.contains('dark'));
const ls = (page: Parameters<typeof waitForList>[0], key: string): Promise<string | null> =>
...
test.describe('design style switcher', () => {
  ...
  const styleButton = (page: Parameters<typeof waitForList>[0], label: string) =>
