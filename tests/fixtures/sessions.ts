 ...
 *
 * This module is the single source of truth: `prepare-fixtures.ts` consumes
...
// ---------------------------------------------------------------------------
// Internal event-spec types (fixture-only, not shared with server)
// ---------------------------------------------------------------------------
...
interface PermissionModeEvent extends BaseEvent {
  kind: 'permission-mode';
}

interface LastPromptEvent extends BaseEvent {
  kind: 'last-prompt';
}

interface AiTitleEvent extends BaseEvent {
  kind: 'ai-title';
  aiTitle: string;
}

interface MalformedEvent extends BaseEvent {
  kind: 'malformed';
}
...
/**
 * Fixture session spec — a superset of Session with additional test-only
 * fields (dir, ageMs, firstUserText, events) used by prepare-fixtures.ts.
 */
...
function buildLongConversation(_topic: string): SpecEvent[] {
...
const byId: Record<string, SessionSpec> = Object.fromEntries(SESSIONS.map((s) => [s.id, s]));
export const session = (key: string): SessionSpec =>
  byId[key] ?? SESSIONS.find((s) => s.title === key) ?? (() => { throw new Error(`No fixture session for key: ${key}`); })();

export const VISIBLE: SessionSpec[] = SESSIONS.filter((s) => s.project !== 'observer-sessions');
export const OBSERVER: SessionSpec[] = SESSIONS.filter((s) => s.project === 'observer-sessions');

const sortByAge = (asc: boolean): string[] =>
  [...VISIBLE].sort((a, b) => (asc ? a.ageMs - b.ageMs : b.ageMs - a.ageMs)).map((s) => s.id);
export const ORDER: { recent: string[]; oldest: string[] } = {
...
export function expectedMessageCount(spec: SessionSpec): number {
 ...
 */
export function expectedTokens(spec: SessionSpec): { tokensIn: number; tokensOut: number } {
  let tokensIn = 0;
  let tokensOut = 0;
...
export function expectedToolCounts(spec: SessionSpec): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of spec.events) {
    if (e.kind === 'assistant' && Array.isArray(e.blocks)) {
      for (const b of e.blocks) {
        if (b.type === 'tool_use' && b.name !== undefined) {
...
export function expectedToolCallCount(spec: SessionSpec): number {
  return Object.values(expectedToolCounts(spec)).reduce((a, b) => a + b, 0);
}

/** Number of tool_result lines flagged as errors. */
export function expectedErrorCount(spec: SessionSpec): number {
  return spec.events.filter((e) => e.kind === 'tool_result' && (e as ToolResultEvent).error).length;
}

/** Visible sessions that recorded at least one tool error. */
export const ERROR_SESSIONS: SessionSpec[] = VISIBLE.filter((s) => expectedErrorCount(s) > 0);

/** The resume command the UI renders for a session. */
export function resumeCommand(spec: SessionSpec): string {
  return `cd "${spec.cwd}" && claude --resume ${spec.id}`;
}

// ---------------------------------------------------------------------------
// Verify fixture specs are compatible with the shared Session shape.
// Pick only the fields that SessionSpec shares with Session so tsc confirms
// fixture data would satisfy the real shape. This is a compile-time check only.
// ---------------------------------------------------------------------------
type _SessionCompatCheck = Pick<
  Session,
  'id' | 'cwd' | 'project' | 'gitBranch' | 'version' | 'model' | 'title'
>;
// If SessionSpec fields diverge from Session, the assignment below will error at compile time.
const _checkSessionCompat: (_s: SessionSpec) => _SessionCompatCheck = (s) => ({
  id: s.id,
  cwd: s.cwd,
  project: s.project,
...
void _checkSessionCompat;

/* ------------------------- JSONL line builders ------------------------- */

let _seq = 0;
const uuid = (id: string, n: number): string => `${id.slice(0, 8)}-evt-${String(n).padStart(4, '0')}`;
...
export function renderSessionJsonl(spec: SessionSpec, nowMs: number): string {
  ...
  spec.events.forEach((evt, i) => {
    ...
    if (obj['__raw'] !== undefined) {
      lines.push(obj['__raw'] as string);
      return;
    }
    if (typeof obj['uuid'] === 'string') prevUuid = obj['uuid'];
